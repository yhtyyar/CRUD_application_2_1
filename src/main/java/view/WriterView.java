...
public class WriterView extends DefaultView{
    ...
    void helpMe() {
        ...
        System.out.println("          Список команд для действий: \n\n"  +
                CREATE + " 'Имя'  'Фамилия' 'Регион(необязательно)' - создание нового пользователя \n" +
                GET_BY_ID + " 'ID' - получение одного из писателей по ID \n" +
                GET_ALL + " - получение данных про всех писателей \n" +
                EDIT_BY_ID + "  'ID' 'Имя'  'Фамилия' 'Регион' - изменить данные писателя по ID \n" +
    ...
    static DefaultView getInstance() {
        if (defaultView == null) {
            defaultView = new WriterView();
        }
        return defaultView;
    }
}
